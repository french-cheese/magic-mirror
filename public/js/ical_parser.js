...
function ical_parser(feed_url, callback) {
	...
	this.makeDate = function (ical_date) {
		...
		dt.monthname = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November",
			"December"][dt.date.getMonth()];
	...
	this.parseICAL = function (data) {
		...
		for (var i = 0; i < cal_array.length; i++) {
			...
			// Si on rencontre une alarme
			if (!in_alarm && ln == 'BEGIN:VALARM') {
				in_alarm = true;
			}
			if (in_alarm && ln == 'END:VALARM') {
				in_alarm = false;
			}
